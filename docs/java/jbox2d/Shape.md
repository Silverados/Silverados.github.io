...
#### 形状定义
定义起点和终点即可定义一条线段。
- m_vertex1, m_vertex2: 是顶点数据。
- m_vertex0: 是m_vertex1的前一个顶点
- m_m_vertex3: 是m_vertex2的后一个顶点。
- m_hasVertex0和m_hasVertex3是标识邻接顶点是否生效的字段。
...
给线段一点宽度。
```java
m_radius = Settings.polygonRadius = 0.01f;
```
##### 克隆方法
```java
  public Shape clone() {
...
### ChainShape
链形状 (ChainShape) 是一系列线段的自由形式序列。链形状具有双面碰撞，因此可以使用内侧和外侧碰撞。你可以使用任何绕组顺序。连通性信息用于创建平滑的碰撞。警告：如果链形状存在自相交情况，碰撞将无法正常进行。
